...
{  
   ...
   //初始化TIM3
	...
	TIM_OC2PreloadConfig(TIM3, TIM_OCPreload_Enable);  //使能TIM3在CCR2上的预装载寄存器	TIM_OC1PreloadConfig(TIM3, TIM_OCPreload_Enable);
...
{  
	...
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_0;       //PA8为定时器1的pwm波输出口
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF_PP; //设置该引脚为复用输出功能,输出TIM1 CH1的PWM脉冲波形
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
...
{  
	...
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_8;       //PA8为定时器1的pwm波输出口
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF_PP; //设置该引脚为复用输出功能,输出TIM1 CH1的PWM脉冲波形
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
	GPIO_Init(GPIOA, &GPIO_InitStructure);
